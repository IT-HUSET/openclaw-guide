...
Scans incoming channel messages (WhatsApp, Signal, Google Chat) for prompt injection using a local DeBERTa ONNX model. Companion to [web-guard](../web-guard/) which protects the web_fetch tool — channel-guard protects the **inbound message** surface instead.
