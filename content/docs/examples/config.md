...
```json5
{
  ...
  "plugins": {
    "entries": {
      ...
      // agent-guard: Not needed for 2-agent config (no inter-agent exec boundary).
      // Add it when using the hardened variant with a computer agent —
      // see hardened-multi-agent.md for config.
      "image-gen": {
        "enabled": true,
        "config": {
