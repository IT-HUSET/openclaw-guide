...
>
> **Version note (2026.2.21):** Shell startup-file env injection blocked across all exec paths — `BASH_ENV`, `ENV`, `BASH_FUNC_*`, `LD_*`, and `DYLD_*` are stripped from the exec environment before command execution. Heredoc body expansion tokens blocked in shell allowlist analysis; `system.run` allowlist evaluated per shell segment (prevents chained-command bypasses). Gateway Control UI now requires secure context and paired-device checks even when `gateway.controlUi.allowInsecureAuth` is set. Tailscale `X-Forwarded-For` header auth scoped to Control UI WebSocket only — HTTP gateway routes still require explicit token/password. Sandbox browser containers hardened: `--no-sandbox` removed from container entrypoint (opt-in via `OPENCLAW_BROWSER_NO_SANDBOX` env var), noVNC observer sessions require VNC password, default Docker network changed to `openclaw-sandbox-browser`. TTS provider switching now opt-in: `messages.tts.modelOverrides.allowProvider` defaults to `false`.
>
> **Version note (2026.2.22):** `openclaw config get` now redacts sensitive credential values before printing — config paths are shown but secret values are masked. `tools.exec.safeBins` no longer trusts PATH-derived directories; use `tools.exec.safeBinTrustedDirs` to declare explicitly trusted directories, with exec pinned to resolved absolute paths. `sessions_history` tool output redacts sensitive token patterns. `groupPolicy: "allowlist"` without explicit `groups` now fails closed across all message channels (previously could inherit permissive defaults). Shell-wrapper env overrides restricted: `SHELLOPTS`, `PS4`, `HOME`, and `ZDOTDIR` blocked in exec env sanitizers. New `openclaw security audit` findings: `security.exposure.open_groups_with_runtime_or_fs` (open group policies exposing runtime/FS tools without sandbox or workspace guard) and `gateway.nodes.allow_commands_dangerous` (risky `gateway.nodes.allowCommands` overrides). SSRF blocking expanded to additional RFC special-use IPv4 ranges (benchmarking 198.18.0.0/15, TEST-NET, multicast, reserved/broadcast blocks).
