...
> **Env var precedence** (highest wins): process environment → CWD `.env` → `~/.openclaw/.env` → config `env` block → shell import. See [official docs](https://docs.openclaw.ai/help/environment) for details.
...
> **Security — Docker gateway container hardening (2026.5.5+):** The bundled `docker-compose.yml` generated by `docker-setup.sh` now drops `NET_RAW` and `NET_ADMIN` Linux capabilities and sets `no-new-privileges: true` on the gateway container. If you are running an older generated compose file, regenerate it or add these settings manually under `services.gateway.cap_drop` and `services.gateway.security_opt`.
