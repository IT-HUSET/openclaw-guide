...
| 2026.7.33–2026.7.35 | Extended-stable maintenance line: exec escaped-newline command injection hardening, browser Origin checks before `auth.mode: "none"` on HTTP, plugin Git install argument-injection hardening, `openclaw backup create` writes archives with owner-only `0o600` permissions, webhook/phone-number/model-status log redaction, malformed media/node payload rejection, Doctor plugin registry preservation on repair | No new release-line features; security/reliability backports only — see [Phase 3](phases/phase-3-security.md) version note |
